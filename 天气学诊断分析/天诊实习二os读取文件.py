'''
天诊实习 2，利用 os 遍历，读取文件，并存到一个 nc 文件中
'''
import os
...
for var_i in varname:
    ...
    for root, dirs, files in os.walk(file):  #####当前路径，路径下的子文件夹，文件
        a = np.zeros(len(dirs))  #####让 dirs 里高度文件夹按数值大小排序
        for i in dirs:
        ...
        if not var_i == 'uv':
            for f in files:
                ...
                hh = np.delete(hh, list(range(45, 50)), axis=1)  ####去掉末尾的 nan 值
                aaa.append(hh)  ####列表追加
        else:
            for f in files:
                data_air = pd.read_csv(os.path.join(root, f), skiprows=3, header=None, sep='\s+')
                hh = data_air.values.reshape(2, 29, 50)
                hh = np.delete(hh, list(range(45, 50)), axis=2)  ####去掉末尾的 nan 值
                bbb.append(hh)  ####列表追加
    if var_i == 'uv':
        bbb_array = np.array(bbb)  #####把列表 bbb 转化成数组
        bbb_array = bbb_array.reshape(6, 17, 2, 29, 45)
        bbb_array = bbb_array[::-1, :, :, ::-1, :]  #####反转 y 轴和高度轴
    else:
        aaa_array = np.array(aaa)  #####把列表 bbb 转化成数组
        aaa_array = aaa_array.reshape(6, 17, 29, 45)
        aaa_array = aaa_array[::-1, :, ::-1, :]  #####反转 y 轴和高度轴
