...
for i in benfile:
    if (".py" in i) or (".ipynb" in i):
        ...
        if (not os.path.exists('D:\WORKcode\pythoncode\python_practice\台风相关绘图\\'+i.split('\\')[-1])) or (not filecmp.cmp(i,'D:\WORKcode\pythoncode\python_practice\台风相关绘图\\'+i.split('\\')[-1])):
            print('add',i)
